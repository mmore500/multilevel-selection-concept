...
To perform our statistical test for deviation from expected, we perform a bootstrap test of the mean percentile ranking against the null 50th percentile expectation.
We used a bootstramp sample size of TODO.
As a caveat, bootstrap testing is known to be unreliable for small sample sizes (e.g., $\sim20$ observations); thus, where reporting bootstrap results we indicate where this condition is violated.
Another notable subtlety of this approach is that comparator groups should decompose the set of available MDCs into discrete, consistent partitions.
...
This procedure ensures that comparing againt the 50th percentile is appropriate because that is the true group mean.
...
A variety of methods exist for statistical tests of elevated mutation frequency \citep{TODO}.
As we found that the primary challenge in accomplishing effective detection of multilevel selection effects is in testing for group-level fitness effects (e.g., transmission efficacy) is the fitness effect component of the screen, we adopted a simple approach to screening mutation frequency for the purposes of present work.

To begin, we generated a simple calculation of baseline mutation frequency $\lambda$ as the mean number of net observed non-focal MDCs per instantiatwd non-focal background site.
To assess the significance of the observed mutation count in deviating from this expectation, we applied the Poisson survival function to calculate the probability of observing an equivalent or more extreme mutation count under the assumption of mutations being distributed $\sim \mathsf{Pois}(\lambda)$.

\subsection{Covasim Agent-based Epidemiology Simulation Platform}
...
Five replicate simulation trials were conducted per observation, except for some large-scale simulation trials which observations were downsized to comprise three replicates.
...
Experiments assumed the wildtype TODO strain, except for multi-strain experiments which additionally incorporate extrinsic imports of alpha, beta, and gamma strains at the approximate dates where they were first observed in the course of the Covid-19 pandemic.
Due to practical limitations, a baseline population size of 200,000 agents was used in most cases.
However, to assess the effect of dataset size on detection sensitivity, some experiments were replicated with a larger population size of 1.2 million agents.
...
In this manner, we sought to capture Luria–Delbruck dybamics where mutants that arose by chance early in an infection had a greater impact on population composition than mutants arising later on \citep{TODO}.
...
Figure \ref{fig:phylos-vanilla-example} shows sample pathogen phylogenies observed in baseline condition experiments.
...
To characterize , we report further details on average case counts and focal MDC/mutant counts in our supplementary material \citep{TODO}.
