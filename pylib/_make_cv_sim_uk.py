...
def make_cv_sim_uk(
    *,
    ...
    data_end = "2021-10-31"
    # Final date for calibration -- set this to a date before boosters started
    ...
    # For simplicity, we assume linear scale-up for each age group,
    # from 0% vax coverage to the final uptake value over the duration of the
    # vaccination campaign
    def set_subtargets(vx_phase):
        return lambda sim: cv.true(
            (sim.people.age >= vx_phase["start_age"])
            * (sim.people.age < vx_phase["end_age"])
        )
    ...
    def make_sim(pop_size, seed, beta, verbose=0.1):

        # Set the parameters
        # total_pop    = 67.86e6 # UK population size
        total_pop = 55.98e6  # UK population size
        # Actual simulated population
        ...
        sim = cv.Sim(pars=pars, location="uk")

        # ADD BETA INTERVENTIONS
        # sbv is transmission in schools and assumed to be 63%=0.7*90% assuming
        # that masks are used and redyce it by 30%
        # from June 2021 we will asume that it is 50% as a combination of large
        # scale isolation of bubbles - found via seeking optimal value
        sbv = 0.63
        sbv_new = 0.63
        beta_dict = sc.odict(
            {
                ...
                # full relaxing of social distancing - step 4 -
                ...
                # to fit data we need to increase testing
                # and mixing from 22/06/2021
        ...
        tc_day = sim.day(
            "2020-03-16"
        )  # intervention of some testing (tc) starts on 16th March
        # and we run until 1st April when it increases
        te_day = sim.day(
            "2020-04-01"
        )  # intervention of some testing (te) starts on 1st April
        # and we run until 1st May when it increases
        tt_day = sim.day(
            "2020-05-01"
        )  # intervention of increased testing (tt) starts on 1st May
        tti_day = sim.day(
            "2020-06-01"
        )  # intervention of tracing and enhanced testing (tti)
        # starts on 1st June
        tti_day_july = sim.day(
            "2020-07-01"
        )  # intervention of tracing and enhanced testing (tti)
        # at different levels starts on 1st July
        tti_day_august = sim.day(
            "2020-08-01"
        )  # intervention of tracing and enhanced testing (tti)
        # at different levels starts on 1st August
        ...
        # to match the increase in June-mid July from optuna
        # increased testing is necessary
        s_prob_june21 = 0.19769
        # to match the drop in mid-July from optuna decline ine testing
        # (or switched tracing off as alternative)
        ...
        interventions += [
            ...
            cv.test_prob(
                symp_prob=s_prob_sep21,
                asymp_prob=0.008,
                symp_quar_prob=0.0,
                start_day=tti_day_sep21,
                end_day=tti_day_oct21 - 1,
                test_delay=t_delay,
            ),
            cv.test_prob(
                symp_prob=s_prob_oct21,
                asymp_prob=0.004,
                symp_quar_prob=0.0,
                start_day=tti_day_oct21,
                end_day=tti_day_nov21 - 1,
                test_delay=t_delay,
            ),
            cv.test_prob(
                symp_prob=s_prob_nov21,
                asymp_prob=0.008,
                symp_quar_prob=0.0,
                start_day=tti_day_nov21,
                end_day=tti_day_dec21 - 1,
                test_delay=t_delay,
            ),
            cv.test_prob(
                symp_prob=s_prob_dec21,
                asymp_prob=0.008,
                symp_quar_prob=0.0,
                start_day=tti_day_dec21,
                test_delay=t_delay,
            ),
            cv.contact_tracing(
            ...
            cv.dynamic_pars(
                {"iso_factor": {"days": te_day, "vals": iso_vals}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_july, "vals": iso_vals1}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_sep, "vals": iso_vals2}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_oct, "vals": iso_vals3}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_nov, "vals": iso_vals4}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_dec, "vals": iso_vals5}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_march, "vals": iso_vals6}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_june21, "vals": iso_vals7}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_july21, "vals": iso_vals8}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_august21, "vals": iso_vals9}}
            ),
            cv.dynamic_pars(
                {"iso_factor": {"days": tti_day_sep21, "vals": iso_vals10}}
            ),
        ]
        ...
        az_vaccine = sc.mergedicts(
            {"label": "az_uk"}, sc.mergedicts(dose_pars, variant_pars)
        )
        ...
        variant_pars["omicron"] = 1 / 7  # PLACEHOLDER
        pfizer_vaccine = sc.mergedicts(
            {"label": "pfizer_uk"}, sc.mergedicts(dose_pars, variant_pars)
        )
        ...
        for age, vx_phase in vx_rollout.items():
            ...
            vx = cv.vaccinate_prob(
                vaccine=vaccine,
                days=days,
                subtarget=subtarget_dict[age],
                label=f"Vaccinate {age}",
            )
            interventions += [vx]
    ...
    # Make sim
    sim = make_sim(pop_size, seed, beta)

    # Add analyzers
    n_doses = []

    def dose_analyzer(sim):
        return n_doses.append(sim.people.doses.copy())

    age_stats = cv.daily_age_stats(
        states=["vaccinated", "exposed", "severe", "dead"]
    )
