from hstrat import _auxiliary_lib as hstrat_aux
import pandas as pd


def alifestd_mark_num_leaves_sibling_asexual(
    phylogeny_df: pd.DataFrame,
    mutate: bool = False,
) -> pd.DataFrame:
    """Mark the number of leaves descendant from each node's sibling.

    Nodes with no siblings (e.g., root nodes) will have value 0 marked.

    Parameters
    ----------
    phylogeny_df : pd.DataFrame
        Alife standard DataFrame containing the phylogenetic relationships.

    mutate : bool, optional
        If True, modify the input DataFrame in place. Default is False.

    Returns
    -------
    pd.DataFrame
        Phylogeny DataFrame with an additional column "num_leaves_sibling"
    """

    if not mutate:
        phylogeny_df = phylogeny_df.copy()

    phylogeny_df = hstrat_aux.alifestd_try_add_ancestor_id_col(
        phylogeny_df, mutate=True
    )

    if "num_leaves" not in phylogeny_df.columns:
        phylogeny_df = hstrat_aux.alifestd_mark_num_leaves_asexual(
            phylogeny_df, mutate=True
        )

    if hstrat_aux.alifestd_has_contiguous_ids(phylogeny_df):
        phylogeny_df = phylogeny_df.reset_index(drop=True)
    else:
        phylogeny_df = phylogeny_df.set_index("id", keep=True)

    phylogeny_df["num_leaves_sibling"] = (
        phylogeny_df.loc[
            phylogeny_df["ancestor_id"],
            "num_leaves",
        ].values
        - phylogeny_df["num_leaves"].values
    )

    return phylogeny_df
